...
| `tachyon.network.max-pending-sse-bytes` | `0B` | Encoded, unsent output one POST-SSE stream may buffer. `0` disables buffering: a tool waits until each event reaches the socket. Past it, a tool sending progress, logs or comments waits for the client. The final response is always accepted. Takes a `DataSize`. |
